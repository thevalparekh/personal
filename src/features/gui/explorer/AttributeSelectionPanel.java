...
import features.attributeSelection.AttributeSelection;
...
import features.gui.visualize.MatrixPanel;
 ...
 /** Stop the class combo from taking up to much space */
      ...
      public void propertyChange(PropertyChangeEvent e) {
        if (m_AttributeEvaluatorEditor.getValue() instanceof AttributeEvaluator) {
          ...
          if (m_AttributeSearchEditor.getValue() instanceof Ranker) {
            ...
            if (result == JOptionPane.YES_OPTION) {
              m_AttributeSearchEditor.setValue(new features.attributeSelection.GreedyStepwise());
            } else {
              // restore to what was there previously (if possible)
              if (backup != null) {
                m_AttributeEvaluatorEditor.setValue(backup);
	...
	public void propertyChange(PropertyChangeEvent e) {
          if (m_AttributeSearchEditor.getValue() instanceof Ranker) {
            if (!(m_AttributeEvaluatorEditor.getValue() instanceof AttributeEvaluator)) {
              ...
              if (result == JOptionPane.YES_OPTION) {
                m_AttributeEvaluatorEditor.setValue(new features.attributeSelection.InfoGainAttributeEval());
              } else {
                // restore to what was there previously (if possible)
                if (backup != null) {
            ...
            if (m_AttributeEvaluatorEditor.getValue() instanceof AttributeEvaluator) {
              ...
              if (result == JOptionPane.YES_OPTION) {
                m_AttributeEvaluatorEditor.setValue(new features.attributeSelection.CfsSubsetEval());
              } else {
                // restore to what was there previously (if possible)
                if (backup != null) {
                  m_AttributeSearchEditor.setValue(backup);
  ...
  protected void startAttributeSelection() {
	...
	public void run() {
	  ...
	  if (sname.startsWith("features.attributeSelection.")) {
	    name += sname.substring("features.attributeSelection.".length());
	  } else {
	    name += sname;
	  }
	  String ename = evaluator.getClass().getName();
	  if (ename.startsWith("features.attributeSelection.")) {
	    name += (" + "
		     +ename.substring("features.attributeSelection.".length()));
	  } else {
	    name += (" + "+ename);
	  }
	  ...
	  String cmdFilter;
          ...
          // 2. filter command
          features.filters.supervised.attribute.AttributeSelection filter = 
            new features.filters.supervised.attribute.AttributeSelection();
	  ...
	  cmdFilter =   filter.getClass().getName() + " " 
          ...
          features.classifiers.meta.AttributeSelectedClassifier cls = 
            new features.classifiers.meta.AttributeSelectedClassifier();
