...
import features.clusterers.DensityBasedClusterer;
...
public class PredictionAppender
  ...
  protected transient features.gui.Logger m_logger;
...
//  pass on any format that we might have determined so far
  ...
  public void acceptClassifier(IncrementalClassifierEvent e) {
    features.classifiers.Classifier classifier = e.getClassifier();
...
//	    fill in predicted probabilities
  ...
  public void acceptClusterer(BatchClustererEvent e) {
	...
	|| m_testSetListeners.size() > 0) {
      ...
      features.clusterers.Clusterer clusterer = e.getClusterer();
      String test;
  ...
  private Instances 
    makeDataSetProbabilities(Instances format,
			     features.classifiers.Classifier classifier,
  ...
  throws Exception {
    ...
    for (int i = 0; i < format.classAttribute().numValues(); i++) {
      features.filters.unsupervised.attribute.Add addF = new
	features.filters.unsupervised.attribute.Add();
      ...
      newInstances = features.filters.Filter.useFilter(newInstances, addF);
    }
  ...
  throws Exception {
    ...
    Instances newInstances = 
      features.filters.Filter.useFilter(format, addF);
    newInstances.setRelationName(format.relationName()+relationNameModifier);
    return newInstances;
  }
  ...
  private Instances 
    makeClusterDataSetProbabilities(Instances format,
			     features.clusterers.Clusterer clusterer,
			     String relationNameModifier) 
  throws Exception {
    int numOrigAtts = format.numAttributes();
    Instances newInstances = new Instances(format);
   ...
   */
  public void setLog(features.gui.Logger logger) {
    m_logger = logger;
  }
