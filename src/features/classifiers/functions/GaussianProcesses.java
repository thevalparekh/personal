...
package features.classifiers.functions;
...
import features.classifiers.IntervalEstimator;
import features.classifiers.functions.supportVector.Kernel;
import features.classifiers.functions.supportVector.PolyKernel;
import features.classifiers.functions.supportVector.RBFKernel;
...
import features.core.Statistics;
...
import features.filters.unsupervised.attribute.NominalToBinary;
import features.filters.unsupervised.attribute.Normalize;
import features.filters.unsupervised.attribute.ReplaceMissingValues;
import features.filters.unsupervised.attribute.Standardize;
 ...
 *  The Kernel to use.
 *  (default: features.classifiers.functions.supportVector.PolyKernel)</pre>
 * 
 * <pre> 
 * Options specific to kernel features.classifiers.functions.supportVector.RBFKernel:
 * </pre>
 * 
 * <pre> -D
...
public class GaussianProcesses 
  ...
  protected features.core.matrix.Matrix m_C;
    
  /** The vector of target values. */
  protected features.core.matrix.Matrix m_t;
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    m_C = new features.core.matrix.Matrix(insts.numInstances(),insts.numInstances());
    double kv;
    double sum = 0.0;
    ...
    //features.core.matrix.CholeskyDecomposition cd = new features.core.matrix.CholeskyDecomposition(m_C);

    //if (!cd.isSPD())
    //throw new Exception("No semi-positive-definite kernel?!?");

    features.core.matrix.LUDecomposition lu = new features.core.matrix.LUDecomposition(m_C);
    if (!lu.isNonsingular())
	throw new Exception("Singular Matrix?!?");

    features.core.matrix.Matrix iMat = features.core.matrix.Matrix.identity(insts.numInstances(),insts.numInstances());

    m_C = lu.solve(iMat);

    m_t = new features.core.matrix.Matrix(insts.numInstances(),1);
  ...
  public double classifyInstance(Instance inst) throws Exception {
    ...
    features.core.matrix.Matrix k = new features.core.matrix.Matrix(m_NumTrain,1);
  ...
  public Enumeration listOptions() {
    ...
    result.addElement(new Option(
	"\tThe Kernel to use.\n"
	+ "\t(default: features.classifiers.functions.supportVector.PolyKernel)",
