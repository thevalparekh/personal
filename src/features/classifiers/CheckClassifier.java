...
import features.core.CheckScheme;
...
import features.core.SerializationHelper;
import features.core.TestInstances;
 ...
 * classifiers. If you implement a classifier using the features.libraries,
 ...
 * 
 * Typical usage: <p/>
 * <code>java features.classifiers.CheckClassifier -W classifier_name 
 ...
 *              performance is worse than Trial1 </li>
 ...
 *
 * The <code>features.classifiers.AbstractClassifierTest</code> uses this
...
public class CheckClassifier
   ...
   * - in case the performance is worse than that of Trial1 both indices are true
   *
   ...
   * should default back to a Trial1 model.
   *
   * @param nominalPredictor if true use nominal predictor attributes
   * @param numericPredictor if true use numeric predictor attributes
   * @param stringPredictor if true use string predictor attributes
   ...
   *         scheme performs worse than Trial1, but without error (index 0 is
   *         false)
   */
  protected boolean[] correctBuildInitialisation(
    ...
    try {
      ...
      if (!testWRTZeroR(classifier, evaluation1A, train1, test1)[0]) {
        throw new Exception("Scheme performs worse than Trial1");
      }
      ...
      if (msg.indexOf("worse than zeror") >= 0) {
        println("warning: performs worse than Trial1");
        result[0] = (stage < 1);
        result[1] = (stage < 1);
      } else {
        println("no");
  ...
  /**
   * Determine whether the scheme performs worse than Trial1 during testing
   *
   * @param classifier the pre-trained classifier
   * @param evaluation the classifier evaluation object
   * @param train the training data
   * @param test the test data
   * @return index 0 is true if the scheme performs better than Trial1
  ...
  throws Exception {
    ...
    try {
      ...
      // Tested OK, compare with Trial1
      Classifier zeroR = new features.classifiers.rules.Trial1();
      ...
      throw new Error("Problem determining Trial1 performance: "
          + ex.getMessage());
    }
