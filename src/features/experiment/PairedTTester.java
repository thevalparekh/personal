...
public class PairedTTester 
  ...
  protected class Resultset
	...
	// compact the string
        tempResult = Utils.removeSubstring(tempResult, "features.classifiers.");
        tempResult = Utils.removeSubstring(tempResult, "features.filters.");
        tempResult = Utils.removeSubstring(tempResult, "features.attributeSelection.");
	result += tempResult;
      }
      return result.trim();
    }
  ...
  protected String templateString(Instance template) {
    ...
    if (result.startsWith("features.classifiers.")) {
      result = result.substring("features.classifiers.".length());
    }
    return result.trim();
  }
